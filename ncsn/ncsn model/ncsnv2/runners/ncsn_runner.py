...
class NCSNRunner():
    ...
    def fast_fid(self):
        ...
        from evaluation.fid_score import get_fid, get_fid_stats_path,get_inception,get_inception_stats_path
        import pickle
        score = get_model(self.config)
        score = torch.nn.DataParallel(score)
        ...
        for ckpt in tqdm.tqdm(range(self.config.fast_fid.begin_ckpt, self.config.fast_fid.end_ckpt + 1, 5000),
                      desc="processing ckpt"):
          checkpoint_path = os.path.join(self.args.log_path, f'checkpoint_{ckpt}.pth')
          ...
          try:
              states = torch.load(checkpoint_path, map_location=self.config.device)
              ...
              path =get_inception_stats_path(self.args, self.config, download=True)
              print("check3");
             # fid,inception = get_fid(stat_path, output_path)
              ...
              outputpath='exp/inception_samples/cifar10'
              print(outputpath)
              inception=get_inception(path,outputpath)              
              inceptions[ckpt]=inception
              print("ckpt: {}, inception: {}".format(ckpt, inception))
